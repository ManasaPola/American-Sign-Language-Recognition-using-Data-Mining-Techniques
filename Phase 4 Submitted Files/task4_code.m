...
clc;clear;
   ...
   % files=["task4data.csv"];
  %  disp(files(m));
[InputData,Textdata] = xlsread('task4data.csv');
%countaction = [];
...
trainrraylabel=zeros(1200,1);
testrraylabel=zeros(800,1);

for j=(120*(z-1))+((12*(z-1))+1):(120*(z-1))+(12*z)
    trainrraylabel(j)=1;
end
for j=(80*(z-1))+((8*(z-1))+1):(80*(z-1))+(8*z)
    testrraylabel(j)=1;
end
...
% Save file
destfile = strcat('C:\Users\kandu\Documents\DM\Assignmnet4\Tree\',gestures(z)+"_task4",'.fig');
saveas(h,destfile);
close(h);

title(" task4 "+gestures(z));
count=0;
TP=0;
FP=0;
FN=0;
...
for k=(200*(z-1))+(((z-1)*20)+1):(200*(z-1))+(((z-1)*20)+20)
    classlabel(k)=1;
end
...
tempacctree = [gestures(z),'decision tree', num2str(accuracy),'svm',accuracysvm,'precision',precision,'recall',recall,'F1_score',F1_Score];
tempaccsvm=[ gestures(z),'svm',accuracysvm,'precision',precision_SVM,'recall',recall_SVM,'F1_score',F1_Score_SVM];
            if (z==1)
